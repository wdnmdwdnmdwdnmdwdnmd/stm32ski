...
unsigned char SPI_ReadByte(void)
{
    ...
    (void)HAL_SPI_TransmitReceive(&hspi2, &tx, &rx, 1, 100);
    return rx;
    /* TODO: 根据你的硬件实现，下面是软件SPI实现示例 */
#if INTERFACE_MODE == USE_SPI_4LINE
    while (Reset == SPI_GetFlag(M4_SPI1, SpiFlagSendBufferEmpty));
    SPI_SendData8(M4_SPI1, 0xFF);
    while (Reset == SPI_GetFlag(M4_SPI1, SpiFlagReceiveBufferFull));
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        SPI_SCK_HIGH(); /* 拉高时钟线，触发PAN211 SPI发送数据 */
        if (SPI_MOSI_STATUS())
        {
            Value |= 0x01;
        }
...
{
    if(GpioMode == RF_GPIO_MODE_INPUT)
    {
        if(GpioPin < 8)
        {
    ...
    else if(GpioMode == RF_GPIO_MODE_OUTPUT)
    {
        if(GpioPin < 8)
        {
...
{
    RF_ConfigGpio(MODULE_GPIO_RX, RF_GPIO_MODE_OUTPUT);
    RF_ConfigGpio(MODULE_GPIO_TX, RF_GPIO_MODE_OUTPUT);
...
{
    RF_ConfigGpio(MODULE_GPIO_TCXO, RF_GPIO_MODE_OUTPUT);
...
void RF_ExitSleepState(void)
{
    ...
    RF_WritePageReg(3, 0x26, 0xA0);   /* 使能内部电源 */
    RF_DelayUs(100);                  /* 保证实际延时在100us以上 */
 ...
 *        - LO_800M
 */
void RF_SetLoFreq(uint32_t lo)
{
    if(lo == LO_400M)
    {
        RF_WritePageRegBits(0, 0x3D, 1, 0x70);
    }
    else if(lo == LO_800M)
    {
        RF_WritePageRegBits(0, 0x3D, 0, 0x70);
    }
    else
    {
        RF_WritePageRegBits(0, 0x3D, lo>>4, 0x70);
    }
}
...
{
    RF_WritePageRegBits(3, 0x24, RegulatorMode, 0x08);
}
 ...
 */
void RF_SetFreq(uint32_t Frequency)
{
    int i;
    uint32_t Fa, Fb;
    uint32_t Temp = 0;
    ...
    for (i = 0; i < FreqTableNum; i++)
    {
        ...
        {
            RF_WritePageReg(0, 0x40, g_RfFreqTable[i].VcoParam);
            Temp = Frequency * g_RfFreqTable[i].FreqFactor;
            break;
        }
...
void RF_StartCad(uint8_t Threshold, uint8_t Chirps)
{
    ...
    RF_ConfigGpio(MODULE_GPIO_CAD_IRQ, RF_GPIO_MODE_OUTPUT);
 ...
 * @param <Size> 要发送的数据字节数
 * @note 发送完成后会触发TX_DONE中断
 * @note 在发送完成中断中，需调用RF_EnableLdoPa()函数来关闭芯片内部ldo和PA
 */
void RF_TxSinglePkt(uint8_t *Buffer, uint8_t Size)
{
    RF_WriteReg(0x02, RF_STATE_STB3);        /* 进入待机状态 */
    RF_TurnonLdoPA();                        /* 打开LDO和PA */
    RF_TurnonTxAnt();                        /* 打开发射天线开关 */
    RF_SetTxMode(RF_TX_MODE_SINGLE);         /* 设置单次发送模式 */
    RF_WriteReg(0x02, RF_STATE_TX);          /* 进入发射状态 */
    RF_WritePageReg(1, 0x0C, Size);          /* 设置发送数据长度 */
    RF_WritePageRegs(0, 0x01, Buffer, Size); /* 写入数据到FIFO, 写完数据后，TX会自动开始发送 */
...
{
    uint8_t RxLen;
    RxLen = RF_GetRxPayloadLen(); /* 获取接收数据长度 */
    RF_ReadFifo(Buffer, RxLen);   /* 从FIFO中读取接收到的数据 */
    return RxLen;
}
 ...
 */
float RF_GetPktSnr(void)
{
    float PktSnr = 0, SnrVal;
    uint8_t i, Temp[6];
    ...
    SnrVal = ((float)SingalStrength/(float)((uint32_t)2<<(uint32_t)g_RfCfgParams.SpreadingFactor))/(float)NoiseStrength;
...
uint32_t RF_GetTxTimeMs(uint8_t Size)
{
    ...
    PayloadTime = PayloadTime * (cr + 4);
    PayloadTime = PayloadTime + 8;
    PayloadTime = PayloadTime * SymbolTime;
    TotalTime = PreambleTime + PayloadTime;
...
void RF_StartTxContinuousWave(void)
{
    ...
    RF_WriteReg(0x02, RF_STATE_STB3);
    RF_SetTxMode(RF_TX_MODE_CONTINOUS);
    RF_TurnonLdoPA();
    RF_TurnonTxAnt();
    RF_WriteReg(0x02, RF_STATE_TX);
    RF_WritePageReg(1, 0x0C, 1);
    RF_WritePageRegs(0, 0x01, Buffer, 1); /*  Start tx continuous wave */
    // RF_SetPageRegBits(1, 0x1E, 0x01); /* Start tx continuous wave */
    g_RfOperatetate = RF_STATE_TX;
}
...
void RF_StopTxContinuousWave(void)
{
    ...
    RF_TurnoffLdoPA();
    RF_TurnonRxAnt();
    // RF_ResetPageRegBits(1, 0x1E, 0x01); /* Stop tx continuous wave */
    g_RfOperatetate = RF_STATE_STB3;
}
