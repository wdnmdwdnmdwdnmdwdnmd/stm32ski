 ...
 *         低功耗模式下须配置为输入带上拉电阻
 */
#define SPI_CS_HIGH()      PORT_SetBits(SPI_NSS_PORT, SPI_NSS_PIN)     /* 将SPI_CS引脚拉高 */
#define SPI_CS_LOW()       PORT_ResetBits(SPI_NSS_PORT, SPI_NSS_PIN)   /* 将SPI_CS引脚拉低 */
 ...
 */
#define RF_FREQ_DEFAULT            433000000   /* default frequency */
#define RF_SF_DEFAULT              RF_SF7      /* 默认扩频因子 */
#define RF_BW_DEFAULT              RF_BW_500K  /* 默认带宽 */
#define RF_CR_DEFAULT              RF_CR_4_5   /* 默认信道编码率 */
#define RF_CRC_DEFAULT             RF_CRC_ON   /* 默认CRC校验 */
#define RF_LDR_DEFAULT             RF_LDR_OFF  /* 默认低速率模式 */
#define RF_PREAMBLE_DEFAULT        8           /* 默认前导码长度 */
#define RF_IQ_INVERT_DEFAULT       FALSE       /* 默认IQ不反转 */
...
/**
 * @brief PAN3029/3060寄存器页定义
 * - PAN3029/3060的寄存器分为4个页，每个页各有128个寄存器
 */
typedef enum
{
    RF_PAGE0 = 0, //!< Page 0
...
typedef enum
{
    RF_GPIO_MODE_INPUT = 0x00, //!< GPIO input mode
    RF_GPIO_MODE_OUTPUT = 0x01,//!< GPIO output mode
} RfGpioMode_t;

/**
 * @brief PAN3029/3060的发射模式定义
 * - RF_TX_MODE_SINGLE: 单次发送模式,大多数情况下使用此模式
 * - RF_TX_CONTINOUS: 连续发送模式，常用于载波信号发射
 */
typedef enum
{
 ...
 * - USE_DCDC: 使用DCDC供电，电流略小一点，但接收灵敏度略差
 */
typedef enum
{
